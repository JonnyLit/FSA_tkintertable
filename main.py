...
class TablesApp(Frame):
    ...
    def parse_txt_file_and_obtain_the_json(self, filename=None):
        ...
        chr_num_states = []  # list of chars
        #chr_num_states.append(chr(int("".join(lines))))
        ...
        '''TROPPO MACCHINOSO ED ESAGERATO, BASTA UN ESEMPIO SU POPUP PER INDICARE COME POPOLARE CORRETTAMENTE IL FILE .txt
        flag_num_states_more_than_one = 0
        flag_num_states_not_an_integer_or_not_specified = 0
        flag_a_void_space_is_not_void = 0
        flag_state_line_has_more_or_less_than_4_elements = 0
        flag_state_line_has_invalid
        # checking if there are some errors in the file
        try:
            # checking if the groups of delta transitions for every start_state are properly separated by exactly one void line
            flag_the_next_line_is_a_state = 0
            flag_the_next_line_is_void = 0
            for iter_list in range(len(clean_lines)):
                if iter_list == 0:
                    if len(clean_lines[iter_list]) == 1 and isinstance(clean_lines[iter_list][0], int) == 0:
                        flag_num_states_not_an_integer_or_not_specified = 1
                    elif len(clean_lines[iter_list]) > 1:
                        flag_num_states_more_than_one = 1
                    else:
                        flag_the_next_line_is_void = 1
                if flag_the_next_line_is_void == 1:
                    if len(clean_lines[iter_list]) >= 1 and clean_lines[iter_list][0] != '':
                        flag_a_void_space_is_not_void = 1
                    else:
                        flag_the_next_line_is_a_state = 1
                if flag_the_next_line_is_a_state == 1:
                    if len(clean_lines[iter_list]) < 4 or len(clean_lines[iter_list]) > 4:
                        flag_state_line_has_more_or_less_than_4_elements = 1
                    elif len(clean_lines[iter_list]) == 4 and clean_lines[iter_list][1] != (1 or 0) and clean_lines[iter_list][2] != (1 or 0) and clean_lines[iter_list][3] != (1 or 0):
        '''

        dict_start_states = {}  # to populate column 0 with keys (states), and for every key a dictionary of info on isInit, isFinal, isFaulty
        list_start_states = []  # the indexes of the list represent the row of the related the start_state value
        num_states = 1
        list_events = []  # to populate columnlabels with keys (events), and for every key a dictionary of info on isObs, isContr
        dict_deltas = {}
            ...
            while iter_lines < len(clean_lines):
                ...
                if clean_lines[iter_lines][0] == '' and flag_the_next_line_is_a_state == 0:
                    ...
                    dict_start_states.update({current_start_state: {"row": index_row_start_states,
                                                                    "isInit": clean_lines[iter_lines][1],
                                                                    "isFinal": clean_lines[iter_lines][2],
                                                                    "isProhibited": clean_lines[iter_lines][3]}})
                    ...
                    while flag_end_current_start_state == 0:
                        ...
                        if clean_lines[iter_lines][0] not in dict_events:
                            ...
                            dict_events.update({clean_lines[iter_lines][0]: {"column": index_column_events,
                                                                             "isContr": clean_lines[iter_lines][2],
                                                                             "isObs": clean_lines[iter_lines][3],
                                                                             "isFaulty": clean_lines[iter_lines][4]}})
        ...
        while len(self.currenttable.model.columnlabels) <= len(list_events):
            ...
            #from tkintertable.Custom import MyTable
            if dict_events[list_events[i - 1]]["isObs"] == "o":
                print("o")
                #print("dir(self.currenttable): ", dir(self.currenttable))
                print("self.currenttable has attribute setEventAsObservable?", hasattr(self.currenttable, 'setEventAsObservable'))
                my_globals.setEventAsUnobservable(self.currenttable, list_events[i - 1])
            elif dict_events[list_events[i - 1]]["isObs"] == "uo":
                print("uo")
                my_globals.setEventAsUnobservable(self.currenttable, list_events[i - 1])
            if dict_events[list_events[i - 1]]["isContr"] == "c":
                print("c")
                my_globals.setEventAsControllable(self.currenttable, list_events[i - 1])
            elif dict_events[list_events[i - 1]]["isContr"] == "uc":
                print("uc")
                my_globals.setEventAsUncontrollable(self.currenttable, list_events[i - 1])
            if dict_events[list_events[i - 1]]["isFaulty"] == "f":
                print("f")
                my_globals.setEventAsFaulty(self.currenttable, list_events[i - 1])
            elif dict_events[list_events[i - 1]]["isFaulty"] == "uf":
                print("uf")
        ...
        '''
        while len(self.currenttable.model.columnlabels) > len(list_events)+1:
            self.currenttable.model.deleteColumn(self.currenttable.model.columnlabels[-1])
        '''
        ...
        '''
        #self.currenttable.model.columnNames.append("State")
        self.currenttable.model.columnlabels.update({"State": "State"})
        for i in range(1, len(list_events)+1):
            print("i: ", i)
            self.currenttable.model.columnNames.append(list_events[i-1])
            # self.currenttable.model.columnlabels.update({"name"+str(i): list_events[i-1]})
            self.currenttable.model.columnlabels.update({list_events[i-1]: list_events[i-1]})
            self.currenttable.model.relabel_Column(i, list_events[i - 1])
            if dict_events[list_events[i - 1]]["isObs"] == "o":
                print("o")
                self.currenttable.setEventAsObservable(list_events[i - 1])
            elif dict_events[list_events[i - 1]]["isObs"] == "uo":
                print("uo")
                self.currenttable.setEventAsUnobservable(list_events[i - 1])
            if dict_events[list_events[i - 1]]["isContr"] == "c":
                print("c")
                self.currenttable.setEventAsControllable(list_events[i - 1])
            elif dict_events[list_events[i - 1]]["isContr"] == "uc":
                print("uc")
                self.currenttable.setEventAsUncontrollable(list_events[i - 1])
            if dict_events[list_events[i - 1]]["isFaulty"] == "f":
                print("f")
                self.currenttable.setEventAsFaulty(list_events[i - 1])
            elif dict_events[list_events[i - 1]]["isFaulty"] == "uf":
                print("uf")
                self.currenttable.setEventAsUnfaulty(list_events[i - 1])
        print("rename")
        print("self.currenttable.model.columnlabels: ", self.currenttable.model.columnlabels)
        print("self.currenttable.model.columnNames:", self.currenttable.model.columnNames)
        '''
        ...
        for i in range(len(list_start_states)):
            ...
            if dict_start_states[list_start_states[i]]["isInit"] == "0" and dict_start_states[list_start_states[i]][
                "isFinal"] == "0" and dict_start_states[list_start_states[i]]["isProhibited"] == "0":
                self.currenttable.model.setValueAt(list_start_states[i], i, 0)
            elif dict_start_states[list_start_states[i]]["isInit"] == "1" and dict_start_states[list_start_states[i]][
                "isFinal"] == "1" and dict_start_states[list_start_states[i]]["isProhibited"] == "1":
                self.currenttable.model.setValueAt(list_start_states[i] + "_i_f_p", i, 0)
            elif dict_start_states[list_start_states[i]]["isInit"] == "1" and dict_start_states[list_start_states[i]][
                "isFinal"] == "1" and dict_start_states[list_start_states[i]]["isProhibited"] == "0":
                self.currenttable.model.setValueAt(list_start_states[i] + "_i_f", i, 0)
            elif dict_start_states[list_start_states[i]]["isInit"] == "1" and dict_start_states[list_start_states[i]][
                "isFinal"] == "0" and dict_start_states[list_start_states[i]]["isProhibited"] == "1":
                self.currenttable.model.setValueAt(list_start_states[i] + "_i_p", i, 0)
            elif dict_start_states[list_start_states[i]]["isInit"] == "0" and dict_start_states[list_start_states[i]][
                "isFinal"] == "1" and dict_start_states[list_start_states[i]]["isProhibited"] == "1":
        ...
        '''
        dizionario_start_states         key = start_state , value = row
        list_start_states               list_start_state[row] = state
        dizionario_events               key = event , value = column
        list_events                     list_events[column] = event
        dizionario_deltas               key = numero [0, last_delta-1] , value = {row = start_state , column = event , end_state/s = string_concat}
        '''

        '''
        i = 1
        chr_num_states.append(chr(int(fd.read(1))))
        flag_num_states_done = 0
        while flag_num_states_done == 0:
            if chr_num_states[i] and chr_num_states[i] != 32 and chr_num_states[i] != 13 and chr_num_states[i] != 10:
                i += 1
                chr_num_states.append(chr(int(fd.read(1))))

        str_num_states = "".join(chr_num_states)
        num_states = int(str_num_states)
        print("num_states: ", num_states)
        for line in lines:
            count += 1
        ...
        print(file_content)
        for i in range(20):
            print(str(i)+": '" + str(file_content[i]) + "'")

        # print(bytes_file_content)
        # file_content = str(bytes_file_content)
        # bytes_file_content.decode("ascii")
        file_content.decode("utf-8")
        file_content_str = str(file_content, encoding="utf-8")
        # file_content.encode()
        file_content_str.replace("0", "42")
        ...
        print(file_content[6])

        self.currenttable.model.relabel_Column(1, "a")
        self.currenttable.setEventAsObservable("a")
        self.currenttable.setEventAsControllable("a")
        self.currenttable.model.relabel_Column(2, "b")
        self.currenttable.setEventAsObservable("b")
        self.currenttable.setEventAsControllable("b")
        self.currenttable.model.autoAddColumns(3)
        self.currenttable.model.setValueAt("42", 2, 2)
        global dictcolObservableEvents
        global dictcolControllableEvents
        print("columnlabels:", self.currenttable.model.columnlabels.values())

        dictcolObservableEvents = {}
        dictcolControllableEvents = {}
        for key in self.currenttable.model.columnlabels:
            dictcolObservableEvents[str(self.currenttable.model.columnlabels.get(key))] = "1"
            dictcolControllableEvents[str(self.currenttable.model.columnlabels.get(key))] = "1"

        print(dictcolObservableEvents)
        print(dictcolControllableEvents)
    ...
    def parse_json_file_and_populate_the_table(self, filename=None):
        ...
        while len(self.currenttable.model.columnlabels) <= len(list_events):
            ...
            from tkintertable.Tables import TableCanvas
            if dict_events[list_events[i - 1]]["isObs"] == "1":
                print("o")
                my_globals.setEventAsObservable(self.currenttable, list_events[i - 1])
            elif dict_events[list_events[i - 1]]["isObs"] == "0":
                print("uo")
                my_globals.setEventAsUnobservable(self.currenttable, list_events[i - 1])
            if dict_events[list_events[i - 1]]["isContr"] == "1":
                print("c")
                my_globals.setEventAsControllable(self.currenttable, list_events[i - 1])
            elif dict_events[list_events[i - 1]]["isContr"] == "0":
                print("uc")
                my_globals.setEventAsUncontrollable(self.currenttable, list_events[i - 1])
            if dict_events[list_events[i - 1]]["isFaulty"] == "1":
                print("f")
                my_globals.setEventAsFaulty(self.currenttable, list_events[i - 1])
            elif dict_events[list_events[i - 1]]["isFaulty"] == "0":
                print("uf")
                my_globals.setEventAsUnfaulty(self.currenttable, list_events[i - 1])
            i += 1
        ...
        for i in range(1,len(clean_lines[0])):
            ...
            if clean_lines[0][i] and clean_lines[0][i] != '_':
                if current_event.endswith("_uc_f_uo") or current_event.endswith("_uc_uo_f") or current_event.endswith(
                    ...
                    dict_event_properties.update({current_event: {"isObs": 0, "isContr": 0, "isFaulty": 1}})
                elif current_event.endswith("_uc_f"):
                    current_event = current_event.replace("_uc_f", "")
                    current_event.replace(" ", "")
                    dict_event_properties.update({current_event: {"isObs": 1, "isContr": 0, "isFaulty": 1}})
                    ...
                    current_event.replace(" ", "")
                    dict_event_properties.update({current_event: {"isObs": 0, "isContr": 0, "isFaulty": 0}})
                    ...
                    current_event.replace(" ", "")
                    dict_event_properties.update({current_event: {"isObs": 0, "isContr": 1, "isFaulty": 1}})
                    ...
                    current_event.replace(" ", "")
                    dict_event_properties.update({current_event: {"isObs": 1, "isContr": 0, "isFaulty": 0}})
                elif current_event.endswith("_f"):
                    current_event = current_event.replace("_f", "")
                    current_event.replace(" ", "")
                    dict_event_properties.update({current_event: {"isObs": 1, "isContr": 1, "isFaulty": 1}})
                elif current_event.endswith("_uo"):
                    current_event = current_event.replace("_uo", "")
                    current_event.replace(" ", "")
                    dict_event_properties.update({current_event: {"isObs": 0, "isContr": 1, "isFaulty": 0}})
                else:
                    current_event.replace(" ", "")
                    dict_event_properties.update({current_event: {"isObs": 1, "isContr": 1, "isFaulty": 0}})
    ...
    def open_popup_errors_on_txt_file(self, win=None):
        ...
        label = Label(self.error_win,image=logo)
        label.image = logo
        ...
        '''
        img.zoom(2)
        label = Label(self.ab_win,image=img)
        label.image = img
        label.grid(row=0,column=0,sticky='news',padx=5,pady=5)
        '''
        ...
        '''
        scrollb = Scrollbar(self.ab_win)
        scrollb.activate(index="slider")
        '''
        ...
        top = Toplevel(win)  # win is a Tk object instantiated with:      
        win = Tk()
        top.geometry("750x250")
        top.title("Syntax example .txt file")
        ...
        img = PhotoImage(format='png', file='./heart.png', height=200, width=200)
