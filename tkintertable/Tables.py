                    ...
                    if self.model.getCellRecord(iter_row, iter_col) != None:
                        ...
                        if iter_col == 0:
                            if current_cell[0] and current_cell[0] != '_':
                                if current_cell.endswith("_i_f_p") or current_cell.endswith("_i_p_f") or current_cell.endswith("_f_i_p") or current_cell.endswith("_f_p_i") or current_cell.endswith("_p_f_i") or current_cell.endswith("_p_i_f"):
                                    ...
                                    dict_X.update({str(current_state): {"isInit": "1", "isFinal": "1", "isProhibited": "1"}})
                                elif current_cell.endswith("_i_f"):
                                    current_state = current_cell.replace("_i_f", "")
                                    current_state.replace(" ", "")
                                    dict_X.update({str(current_state):{"isInit": "1", "isFinal": "1", "isProhibited": "0"}})
                                    ...
                                    current_state.replace(" ", "")
                                    dict_X.update({str(current_state):{"isInit": "1", "isFinal": "0", "isProhibited": "1"}})
                                    ...
                                    current_state.replace(" ", "")
                                    dict_X.update({str(current_state):{"isInit": "0", "isFinal": "1", "isProhibited": "1"}})
                                    ...
                                    current_state.replace(" ", "")
                                    dict_X.update({str(current_state):{"isInit": "1", "isFinal": "0", "isProhibited": "0"}})
                                elif current_cell.endswith("_f"):
                                    current_state = current_cell.replace("_f", "")
                                    current_state.replace(" ", "")
                                    dict_X.update({str(current_state):{"isInit": "0", "isFinal": "1", "isProhibited": "0"}})
                                elif current_cell.endswith("_p"):
                                    current_state = current_cell.replace("_p", "")
                                    current_state.replace(" ", "")
                                    dict_X.update({str(current_state):{"isInit": "0", "isFinal": "0", "isProhibited": "1"}})
                                else:
                                    current_state = current_cell
                                    current_state.replace(" ", "")
                                    dict_X.update({str(current_state):{"isInit": "0", "isFinal": "0", "isProhibited": "0"}})
                            else:
                            ...
                            for i in range(len(current_delta_ends)):
                                dict_delta.update({str(chr(iter_ascii_delta)):{"start": str(current_state), "name": str(self.model.getColumnLabel(iter_col)), "ends": str(current_delta_ends[i])}})
                                print("dict_delta", dict_delta)
                                current_key_event = str(self.model.getColumnLabel(iter_col))
                                dict_E.update({current_key_event : {"isObs": str(my_globals.dictcolObservableEvents[current_key_event]) , "isContr": str(my_globals.dictcolControllableEvents[current_key_event]) , "isFaulty": str(my_globals.dictcolFaultyEvents[current_key_event])}})
                                iter_ascii_delta += 1
